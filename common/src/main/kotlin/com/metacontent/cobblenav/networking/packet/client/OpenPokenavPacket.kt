...
class OpenPokenavPacket(val os: PokenavOS) : CobblenavNetworkPacket<OpenPokenavPacket> {
    companion object {
        val ID = cobblenavResource("open_pokenav")
        fun decode(buffer: RegistryFriendlyByteBuf) = OpenPokenavPacket(PokenavOS.decode(buffer))
    }
